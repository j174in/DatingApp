        ...
        public async Task<Group?> GetGroupForConnection(string connectionId)
        {
            return await context.Groups
                ...
                .Where(x => x.Connections
                    .Any(connection => connection.ConnectionId == connectionId))
                .FirstOrDefaultAsync();
        }
        ...
        public async Task<PaginatedResults<MessageDto>> GetMessagesForMember(MessageParams messageParams)
        {
            ...
            query = messageParams.Container switch
            {
                "Outbox" => query.Where(x =>
                    x.SenderId == messageParams.MemberId && x.SenderDelete == false),
                _ => query.Where(x =>
                    x.RecipientId == messageParams.MemberId && x.ReceiverDelete == false)
            };
        ...
        public async Task<IReadOnlyList<MessageDto>> GetMessageThread(string currentMemberId, string recipientId)
        {
            ...
            return await context.Messages.Where(x =>
                (x.SenderId == currentMemberId &&
                    x.SenderDelete == false &&
                    x.RecipientId == recipientId) ||
                (x.RecipientId == currentMemberId &&
                    x.ReceiverDelete == false &&
                    x.SenderId == recipientId))
